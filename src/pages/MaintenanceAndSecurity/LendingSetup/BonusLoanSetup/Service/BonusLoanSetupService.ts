...
export const BonusLoanSetupService = {
  ...
  getAllBonusLoans: async (
    ...
    const endpoint = `/bonus-loan-setup${params.toString() ? `?${params.toString()}` : ""}`
    const response = await apiRequest<ApiResponse<GetAllBonusLoansResponse>>("get", endpoint, null, {
      useAuth: true,
      useBranchId: true,
    })
   ...
   */
  getBonusLoanById: async (id: string): Promise<ApiResponse<BonusLoanDetail>> => {
    const endpoint = `/bonus-loan-setup/${id}`
    const response = await apiRequest<ApiResponse<BonusLoanDetail>>("get", endpoint, null, {
      useAuth: true,
      useBranchId: true,
    })
  ...
  createBonusLoan: async (payload: CreateBonusLoanPayload): Promise<ApiResponse<BonusLoan>> => {
    ...
    try {
      const response = await apiRequest<ApiResponse<BonusLoan>>("post", endpoint, payload, {
        useAuth: true,
        useBranchId: true,
      })
   ...
   */
  updateBonusLoan: async (id: string, payload: UpdateBonusLoanPayload): Promise<ApiResponse<BonusLoan>> => {
    const endpoint = `/bonus-loan-setup/${id}`
    try {
      const response = await apiRequest<ApiResponse<BonusLoan>>("put", endpoint, payload, {
        useAuth: true,
        useBranchId: true,
      })
      return response.data
   ...
   */
  updateBonusLoanStatus: async (id: string, payload: UpdateBonusLoanStatusPayload): Promise<ApiResponse<null>> => {
