...
export default function ChartOfAccounts() {
  ...
  const generateCsvFromData = useCallback(() => {
    try {
      const headers = ["Code", "Name", "Classification", "Sub-Grouping", "Normal Balance", "Contra"]
      ...
      accounts.forEach((account) => {
        try {
          ...
          console.warn("Error processing account entry for CSV:", entryError, account)
          csvRows.push(
            [
      ...
      // Add summary row
      csvRows.push(['"TOTAL ACCOUNTS"', `"${accounts.length}"`, '""', '""', '""', '""'].join(","))
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.success("CSV Export Successful", {
          description: "Chart of Accounts have been exported to CSV successfully.",
      ...
      try {
        if (!accounts || accounts.length === 0) {
          toast.error("No Data to Export", {
            description: "There are no accounts to export. Please refresh and try again.",
            duration: 5000,
          })
          return
        }
        ...
        toast.error("CSV Export Failed", {
          description: "Failed to export Chart of Accounts to CSV. Please check the data and try again.",
          duration: 5000,
        })
      }
    } finally {
  ...
  const handleAddAccount = async (newAccount: any) => {
    try {
      const response = await apiRequest<{ status: string; message: string; data: any }>("post", "/coa", newAccount, {
        useAuth: true,
        useBranchId: true,
      })
