...
export default function AddEditEntryDialog({
  ...
  const handleAccountSelection = (index: number, selectedOption: { value: string; label: string } | null) => {
    if (selectedOption) {
  ...
  const getSelectedAccountIds = (): string[] => {
    return items.map((item) => item.coa_id || item.coa?.id).filter(Boolean) as string[]
  }

  const getAvailableAccountsForItem = (currentIndex: number): ChartOfAccount[] => {
    const selectedIds = getSelectedAccountIds()
    const currentItemAccountId = items[currentIndex]?.coa_id || items[currentIndex]?.coa?.id

    return chartOfAccounts.filter((account) => !selectedIds.includes(account.id) || account.id === currentItemAccountId)
  }
          ...
          <div className="max-h-[85vh] overflow-y-auto pr-2 mt-4">
            <DialogHeader>
              <DialogTitle className="text-2xl font-bold">
                {isEditMode ? "View General Journal" : "Add New General Journal"}
              </DialogTitle>
            </DialogHeader>
            ...
            <div className="space-y-4 py-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-gray-600 mt-1">Auto-generated reference number</p>
                ...
                <div className="space-y-2">
                  ...
                  />
                  <p className="text-xs text-gray-600 mt-1">Auto-generated reference Code</p>
  ...
  `}
                  ...
                  <p className="text-xs text-gray-600 mt-1">The date of the transaction</p>
                ...
                <div className="space-y-2">
                  ...
                  />
                  <p className="text-xs text-gray-600 mt-1">A descriptive name for this journal entry</p>
              ...
              <div>
                ...
                <div className="border border-gray-300 rounded-t-md">
                  ...
                  <div className="grid grid-cols-5 gap-3 px-2 py-2 text-xs font-medium text-gray-600 border-b">
                    ...
                    <div className="col-span-2 text-left ml-6">Account Name</div>
                  ...
                  {items.map((item, index) => {
                    ...
                    return (
                      <div key={index} className="grid grid-cols-5 gap-2 px-2 py-2 border-b last:border-b-0">
                        ...
                        <div className="space-y-1">
                          <Select<{ value: string; label: string }>
                            ...
                            isClearable
                            classNamePrefix={
                              hasFieldError(`items.${index}.coa_id`) ? "react-select-error" : "react-select"
                            }
                          ...
                          <Select<{ value: string; label: string }>
                            ...
                            styles={{
                              ...
                              menu: (base) => ({
                                ...base,
                                fontSize: "12px",
                                minWidth: "100px", // Ensure minimum width for readability
                                width: "auto", // Allow width to adjust to content
                                zIndex: 9999,
                              }),
                              menuList: (base) => ({
                                ...base,
                                minWidth: "180px", // Consistent minimum width
                                width: "auto", // Remove the problematic 10% width
                              }),
                              option: (base) => ({
                                ...base,
                                whiteSpace: "nowrap", // Prevent text wrapping
                                overflow: "hidden",
                                textOverflow: "ellipsis", // Add ellipsis for very long names
                ...
                <div className="grid grid-cols-5 gap-4 py-3 border-l-2  border-r-2  border-b-2 border-gray-300 font-semibold pl-2 rounded-b-md">
                  ...
                  <div className="text-center ml-45">{calculateTotalDebit(items).toFixed(2)}</div>
                  <div className="text-center ml-45">{calculateTotalCredit(items).toFixed(2)}</div>
                  <div></div>
                </div>
              </div>
