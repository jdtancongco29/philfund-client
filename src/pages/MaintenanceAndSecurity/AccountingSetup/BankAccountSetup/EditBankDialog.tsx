...
useEffect(() => {
  if (open) {
    ...
    if (initialValues?.coa_id) {
      ...
      setSelectedModuleName(formValues.code); // Placeholder text
    } else {
    ...
    <DialogContent className="sm:max-w-[700px]" aria-describedby={undefined}>
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4 py-4">
        ...
        <div className="space-y-2">
          <label className="text-sm font-medium leading-none">
            ...
            />
            <Button type="button" variant="outline" onClick={() => setShowBranchDialog(true)}>
              Select
            </Button>
            </div>
        ...
        {/* COA */}
        <div className="space-y-2">
          <label className="text-sm font-medium leading-none">Chart of Account</label>
            <div className="flex items-center gap-2">
            <Input
              readOnly
              placeholder="Select Chart of Account"
              value={selectedModuleName || formValues.coa_name}
              className="cursor-pointer"
              onClick={() => setModuleDialogOpen(true)}
            />
            <Button type="button" variant="outline" onClick={() => setModuleDialogOpen(true)}>
              Select
            </Button>
            </div>
        ...
        <div className="space-y-2">
          ...
          <p className="text-sm text-gray-600">Whether this bank account is currently active</p>
      ...
      <BranchSelectionDialog
        ...
        onSelect={(branch) => {
          ...
          handleInputChange("branch_id", branch.id);
      ...
      <COADialog
        ...
        onSelect={(coa) => {
          ...
          setSelectedModuleName(`${coa.name}`);
